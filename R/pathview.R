...
get_kegg_plots <- function(x, genedata, comparison, pw_list, limit, nbin, res.path) {
  
  pathview_pg(gene.data = genedata,
              pathway.id = pw_list[x],
              pathwayName = names(pw_list[x]),
              kegg.dir = res.path,
              species = "hsa",
...
plot_pathview <- function(multiple_data_fc_df, my_pw_list, res.path = "results/pathview"){
  # input multiple_data_fc_df: matrix with columns: entrezid, and FC columns, e.g. FC.prot, FC.pep, FC.uka
  
  # 2. Run pathview for all pathways in union
  ...
  # figure out limits
  mylimit <- multiple_data_fc_df %>% quantile(c(0.25, 0.75), na.rm = T) %>% abs() %>% max() %>% round(1)
  print(paste0('mylimit: ', mylimit))
  
  purrr::map(1:length(my_pw_list),
             get_kegg_plots, genedata = multiple_data_fc_df,
             res.path = res.path,
             pw_list = my_pw_list,
             limit = 1.5, nbin = 10)
...
pathview_pg <- function (gene.data = NULL, cpd.data = NULL, pathway.id, pathwayName, species = "hsa", 
                         ...
                         na.col = "transparent", ...) 
{
  ...
  for (i in 1:npath) {
    ...
    if (kegg.native) {
      pv.pars = keggview.native_pg(plot.data.gene = plot.data.gene, 
                                   cols.ts.gene = cols.ts.gene, plot.data.cpd = plot.data.cpd, 
                                   cols.ts.cpd = cols.ts.cpd, node.data = node.data, 
                                   pathway.name = pathway.name[i], pathwayName = pathwayName, kegg.dir = kegg.dir, 
...
keggview.native_pg <- function (plot.data.gene = NULL, plot.data.cpd = NULL, cols.ts.gene = NULL, 
                                cols.ts.cpd = NULL, node.data, pathway.name, pathwayName, out.suffix = "pathview", 
                                ...
                                key.pos = "topright", ...) 
{
  ...
  for (np in 1:nplots) {
    img.file = paste0(kegg.dir, "/", pathwayName, ".", pathway.name, ".", pn.suffix[np], ".", "png")
    #browser()
